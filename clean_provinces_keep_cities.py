...
for filename in files:
    ...
    # Keep rows if they:
    # 1. Match valid provinces exactly, OR
    # 2. Start with "TP. " (standardized cities)
    keep_mask = (df[province_col].isin(valid_provinces_with_cities)) | (df[province_col].str.startswith('TP. ', na=False))
    ...
    if len(removed_df) > 0:
        print(f"\nRemoved invalid provinces:")
        invalid_counts = removed_df[province_col].value_counts().head(15)
...
print(f"\n✅ Xóa hoàn tất! Giữ lại các dòng có 'Thành phố'")
