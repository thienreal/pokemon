#!/usr/bin/env python3
"""Vietnam Tourism Shop Database Scraper
-------------------------------------------------
Chức năng: Cào dữ liệu điểm mua sắm từ https://csdl.vietnamtourism.gov.vn/shop/
        bao gồm: tên, địa chỉ

Phạm vi:
 - Lấy tên, địa chỉ từ danh sách mua sắm
 - Tự động follow paginated links (?page=N)
 - Dừng khi gặp trang trống hoặc đạt giới hạn --max-pages
 - Xuất ra CSV với UTF-8 BOM

CLI:
    --output FILE       (mặc định shop.csv)
...
BASE_URL = "https://csdl.vietnamtourism.gov.vn/shop"


@dataclass
class Shop:
    """Dữ liệu điểm mua sắm"""
    name: str
    address: str
...
class ShopScraper:
    ...
    def parse_page(self, html: str) -> List[Shop]:
        """Phân tích HTML để lấy danh sách mua sắm"""
        if not html:
            return []
        ...
        # Tìm tất cả các h4 chứa tên điểm mua sắm
        for h4 in soup.find_all('h4'):
            ...
            if name and address:
                shops.append(Shop(name=name, address=address))
        
        return shops

    def scrape(self, output_file: str = "shop.csv") -> List[Shop]:
        """Cào tất cả dữ liệu mua sắm"""
        all_shops: List[Shop] = []
        page = self.start_page
        empty_count = 0

        logging.info("=== Bắt đầu cào dữ liệu điểm mua sắm ===")
        ...
        while True:
            ...
            if not shops:
                ...
                empty_count = 0
                logging.info(f"Trang {page}: {len(shops)} mục")
                all_shops.extend(shops)

                # Checkpoint mỗi 10 trang
                if page % 10 == 0 or page == self.start_page:
                    self._save_csv(all_shops, output_file)
        ...
        logging.info(f"Tổng cộng: {len(all_shops)} điểm mua sắm")

        return all_shops

    def _save_csv(self, shops: List[Shop], filepath: str):
        ...
        try:
            with open(filepath, 'w', newline='', encoding='utf-8-sig') as f:
                ...
                for shop in shops:
                    writer.writerow([shop.name, shop.address])
            
            logging.info(f"Checkpoint: lưu {len(shops)} mục vào {filepath}")
        except IOError as e:
...
def main():
    parser = argparse.ArgumentParser(
        description="Cào dữ liệu điểm mua sắm từ csdl.vietnamtourism.gov.vn"
    )
    parser.add_argument(
        '--output', '-o',
        default='shop.csv',
        help='Tệp output CSV (mặc định: shop.csv)'
    )
    parser.add_argument(
        '--max-pages', '-m',
        type=int,
    ...
    try:
        shops = scraper.scrape(output_file=args.output)
