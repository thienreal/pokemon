#!/usr/bin/env python3
"""Vietnam Tourism Restaurant Database Scraper
-------------------------------------------------
Chức năng: Cào dữ liệu nhà hàng từ https://csdl.vietnamtourism.gov.vn/rest/
        bao gồm: tên, địa chỉ

Phạm vi:
 - Lấy tên, địa chỉ từ danh sách nhà hàng
 - Tự động follow paginated links (?page=N)
 - Dừng khi gặp trang trống hoặc đạt giới hạn --max-pages
 - Xuất ra CSV với UTF-8 BOM

CLI:
    --output FILE       (mặc định restaurant.csv)
...
BASE_URL = "https://csdl.vietnamtourism.gov.vn/rest"
...
class RestaurantScraper:
    ...
    def parse_page(self, html: str) -> List[Restaurant]:
        """Phân tích HTML để lấy danh sách nhà hàng"""
        if not html:
            return []
        ...
        # Tìm tất cả các h4 chứa tên nhà hàng
        for h4 in soup.find_all('h4'):
            ...
            if name and address:
                restaurants.append(Restaurant(name=name, address=address))
        
        return restaurants

    def scrape(self, output_file: str = "restaurant.csv") -> List[Restaurant]:
        """Cào tất cả dữ liệu nhà hàng"""
        all_restaurants: List[Restaurant] = []
        page = self.start_page
        empty_count = 0

        logging.info("=== Bắt đầu cào dữ liệu nhà hàng ===")
        ...
        while True:
            ...
            restaurants = self.parse_page(html)
            ...
            if not restaurants:
                ...
                empty_count = 0
                logging.info(f"Trang {page}: {len(restaurants)} mục")
                all_restaurants.extend(restaurants)

                # Checkpoint mỗi 10 trang
                if page % 10 == 0 or page == self.start_page:
                    self._save_csv(all_restaurants, output_file)
        ...
        logging.info(f"Tổng cộng: {len(all_restaurants)} nhà hàng")

        return all_restaurants

    def _save_csv(self, restaurants: List[Restaurant], filepath: str):
        ...
        try:
            with open(filepath, 'w', newline='', encoding='utf-8-sig') as f:
                ...
                for rest in restaurants:
                    writer.writerow([rest.name, rest.address])
            
            logging.info(f"Checkpoint: lưu {len(restaurants)} mục vào {filepath}")
        except IOError as e:
...
def main():
    parser = argparse.ArgumentParser(
        description="Cào dữ liệu nhà hàng từ csdl.vietnamtourism.gov.vn"
    )
    parser.add_argument(
        '--output', '-o',
        default='restaurant.csv',
        help='Tệp output CSV (mặc định: restaurant.csv)'
    )
    parser.add_argument(
        '--max-pages', '-m',
        type=int,
    ...
    try:
        restaurants = scraper.scrape(output_file=args.output)
